...
数据目录: D:\2026-SP\Data\
  已有数据:
    D:\2026-SP\Data\WorldClim\        (43 tif, 已下载)
    D:\2026-SP\Data\SRTM\             (1 tif, 已下载)
    D:\2026-SP\Data\SoilGrids\        (5 vrt, 核心土壤属性已下载)
    D:\2026-SP\Data\Calculated\       (GDD/SAT/DUL/Slope 等衍生数据)
    D:\2026-SP\Data\CMIP6\            (CO2 浓度)
  待获取:
    中国土壤数据库 (需手动)
    CHELSA 逐日数据 → 计划改用 ERA5-Land
    Figshare 管理数据 (需手动)
    品种性状数据 (模板已建, 待填入)
```
