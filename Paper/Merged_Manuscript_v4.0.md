...
**版本**: v4.1-merge(2026-09-18,W19 外部验证+可检出性框架:回应闫军老师"过拟合/反常识"质疑)
...
**Results**: (1) **Target trap**: the interpolated target yields pooled R2=0.98 although leave-one-out IDW of its 43 source values reaches only R2=0.20-0.27 - circular validation quantified as an upper bound. (2) **Validation trap**: within a domain only ~5× the variogram range wide, the checkerboard family is **structurally a placebo** - its between-block nearest-neighbour distance stays at 2.3-2.4 km regardless of block size, and the optimism gap vanishes not because large blocks grow honest but because large-grid checkerboards silently degenerate into random splitting (gap +0.113 at 0.5× range → -0.006 at 4× range); large gaps arise only from schemes that cut the elevation gradient or environmental continuity (latitude-block +0.288, LOBO +0.276, env-space +0.54). **Range-aware block sizing is powerless in small domains; domain size in range units must be reported alongside block size.** (3) **Representation trap**: the environment is effectively one elevation axis (PC1 = 66% variance, r(PC1, elevation)=-0.94), PC1-5 jointly explain only R2=0.24 of target A, winter-wheat-appropriate thermal features (Tbase=0 °C seasonal GDD, vernalisation days, deep-freeze days, grain-fill heat) add +0.006 on C and exactly 0 on A, and even the process-model baseline's own spatial pattern cannot be captured (all schemes ≤0.20 on A) - static covariates, same-sensor features, and correctly-parameterised dynamic aggregates all fail. (4) **External validation and detectability (W19)**: on the full Xiao et al. (2024) North China Plain History panel aggregated to the same lattice (7,784 grids; WorldClim+SoilGrids features; SRTM omitted as Pinggu-only), **the identical framework recovers R2=0.690±0.006 under random CV** - environmental covariates do explain ~70% of yield variation across real environmental gradients. Sliding Pinggu-sized windows (n=60) over the same dataset collapse to a **median R2=0.163** (IQR 0.027-0.272; 60% of windows <0.2; minimum -0.466): a near-null is the *norm*, not the anomaly, at this domain size. A power analysis on the real 43-grid feature matrix (synthetic signals of known strength, identical CV pipeline) shows detection probability is low even for true R2=0.3 - the Pinggu null is therefore a **detectability outcome of a small, low-variance domain**, not evidence against environmental control of yield. (5) QRF intervals (PICP 0.705→0.862 after conformal calibration; MPW 0.62-0.79 t/ha ≈ 2.6× target SD) support only trend screening, not insurance-grade pricing - a decision-suitability matrix is provided.

**Conclusions**: For model-derived targets, apparent skill is manufactured by target construction, validation design, and representation limits - not by environmental information, **which our own framework confirms to be large (R2=0.69) wherever environmental gradients are wide enough to detect**. We release a transferable **pre-mapping audit protocol**: (i) declare target provenance and its training fit; (ii) estimate the variogram range **and the domain size in range units** before choosing block sizes - if the domain spans ≲5 ranges, checkerboard/block families are structurally uninformative and gradient-stripe, LOBO, or environment-space designs must be used instead; (iii) test the elevation/PC1 axis before celebrating multi-variable importance; (iv) report interval width against the decision it must support; and (v) **benchmark the achievable signal ceiling of the sampling geometry itself (domain size × environmental variance × n) before interpreting a null as "no environmental signal."**
...
- **6.6 外部验证与可检出性框架【W19 新增,回应"环境贡献应该很大"的导师质疑】**:
  - 6.6.1 **全梯度验证**:Xiao2024 华北平原 History 面板(444,289 个 1km 网格)聚合到同一 0.0417° 格网(7,784 格),同族特征(WorldClim 19 项+SoilGrids 6 项;SRTM 仅覆盖平谷故省略)、同一框架--随机 CV pooled R2=**0.690±0.006**(n=7,784);仅用与平谷共享的 25 特征子集时 R2=0.689(排除特征集差异解释);**1km 细格层面(2 万格抽样)R2=0.450**--聚合平滑抬升 R2(0.45→0.69,MAUP 效应)但细格信号依然强,非聚合伪影(w19_external_validation.csv / w19_finelevel_check.csv);checker 0.625 / 纬度块 0.371,衰减模式与主线一致。**环境协变量在大梯度上解释近 70% 产量变异--框架不是信号毁灭器**
  - 6.6.2 **平谷同尺寸窗口普查**:0.7°×0.5° 窗口(平谷足迹,80-200 格)在华北平原上滑动 60 个--R2 中位 **0.163**,IQR [0.027,0.272],最小 -0.466,60% 窗口<0.2:**近零结果是该域尺寸的常态而非平谷特例**(w19_window_distribution.csv)
  - 6.6.3 **统计功效分析 v2(现实信号形态)**:以全国响应模型(R2=0.69)在平谷 43 格上的预测作为植入信号(与 4 月温度/降水特征相关 r≈0.7,物理良构),按 R2pop=0.05-0.50 缩放后用同一 CV 管线测检出率--**XGBoost pooled 口径检出率对全部信号强度均为 0.00**(n=34 训练样本下深度树纯拟合噪声,median OOF R2≈-0.05);**Ridge 对照:折级检出率 40%(R2pop=0.3),pooled 口径仅 3%**--折级可感知、池化即稀释。结论:(a)"R2≈0"对 n=43 是无信息量的(低于采样几何的可检出上界);(b) **模型复杂度必须匹配样本量**--树模型在该样本量下对平滑区域信号零功效,简单正则模型仅折级敏感;据此,在该数据上"让模型正确预测"的唯一诚实路径是扩大采样,而非更换模型(w19_power_analysis_v2.csv;诊断脚本 scratch/w19f_diag.py)
  - 6.6.4 **尺度依赖谱线**:bio1 网格间 SD 0.99°C(全梯度)→0.694 / 0.57°C→0.656 / 0.45°C→0.580;域尺寸维度:全梯度 7,784 格→0.690 / 平谷同尺寸窗口(80-200 格,60 个)→中位 0.163 / 平谷 43 格→≈0(目标 CV 仅 3.2%)--**环境贡献不消失,可观测信号窗口随环境变异幅度与样本规模双向收缩**;与 G×E 文献的尺度依赖性对话(大梯度气候主导、县域管理主导)
  - 6.6.5 协议增补第 (v) 步:解读任何零结果前,先基准化采样几何自身的信号天花板(域尺度×环境变异×n)
...
- 8.4 **Pre-mapping audit protocol(五步核查协议,转 Supplementary S1 九步自评表)**:
  (i) 目标溯源声明+训练拟合披露;(ii) variogram 变程先估、块尺寸按比例设计;(iii) PC1/海拔轴先验检验再谈多变量重要性;(iv) 区间宽度对照决策需求;(v)【W19】采样几何信号天花板基准化(域尺度×环境变异×n 的可检出上界),零结果须先过此关再作结论
- 8.5 局限:单年单区(跨年待 Zenodo 多年份);无 ground-truth(本文的研究动机与结论边界,不是疏忽);管理变量缺失(假说检验路径 a/b);QRF 校准演示级;【W19 声明】外部验证与平谷主线共用 Xiao2024 过程模型源(同源不同子域),目标仍为模型产物
